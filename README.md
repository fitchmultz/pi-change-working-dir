...
- If an active directory is later deleted or loses access, relative/defaulted path inputs and absolute paths inside it are rejected before file tools can recreate it. Restore it or select an accessible directory with `change_dir` or `/cwd`. Explicit absolute paths outside the unavailable directory remain usable.
